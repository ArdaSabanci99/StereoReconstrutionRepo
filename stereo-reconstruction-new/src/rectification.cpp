...
RectifyResult rectifyLoopZhang(const cv::Mat& left, const cv::Mat& right,
    ...
    // 1. Compute epipoles
    //   e1: null(F)   → F e1 = 0   (left  epipole, seen from left  image)
    //   e2: null(F^T) → F^T e2 = 0 (right epipole, seen from right image)
...
int main(int argc, char** argv) {
    ...
    RectifyResult res = manual ? rectifyLoopZhang(imgL, imgR, calib.F, in_l, in_r)
